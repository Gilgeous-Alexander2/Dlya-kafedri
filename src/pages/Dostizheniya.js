...
export default function Dostizheniya() {
    ...
    return <div ref={ref}>
        ...
        <div style={{ display: 'flex', gap: '100px', flexDirection: 'column', alignItems: 'center' }}>
            {nomer.map((el, i) => (
                <div key={i} style={{ display: 'flex', gap: '100px' }}>
                    <motion.div initial={'hidden'}
                        animate={isInView ? 'show' : 'none'}
                        variants={anim}
                        custom={i}
                    ><Kartocka nomer={nomer[i][0]} dostizh={Dostizh[nomer[i][0]-1]} text={Text[nomer[i][0]-1]}></Kartocka></motion.div>
                    <motion.div initial={'hidden'}
                        animate={isInView ? 'show2' : 'none'}
                        variants={anim}
                        custom={i}>
                        <Kartocka nomer={nomer[i][1]} dostizh={Dostizh[nomer[i][1]-1]} text={Text[nomer[i][1]-1]}></Kartocka></motion.div>
                </div>))
            }
